...
public class Packet {
	...
	private byte[] timestamp = new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00 };
	...
	public Packet(byte[] targetMac, byte[] gatewayMac, byte[] timestamp,
			byte[] type) {
	...
	public byte[] toByteArray() {
		...
		try {
			outputStream.write(new byte[] { (byte) (0x24 + payload.length),
					0x00 });
	...
	public static Packet fromByteArray(byte[] data) {
		...
		if (data.length > 31) {
			result.setPayload(ArrayUtils.subarray(data, 31, data.length));
		}
		return result;
	}
